<template>
  <div class="page-login">
    ...
    <van-cell-group>
      <!-- 输入框 -->
      <van-field required @blur="checkMobile" :error-message="errMsg.mobile" v-model="loginForm.mobile" label="手机号" placeholder="请输入手机号" />
      <van-field required @blur="checkCode" :error-message='errMsg.code' v-model="loginForm.code" label="验证码" placeholder="请输入验证码">
        <van-button class="p5" slot="button" size="mini" type="primary">发送验证码</van-button>
      </van-field>
    </van-cell-group>
    <div class="btn_box">
      <van-button type="info" @click="subLogin" block round>登 录</van-button>
    </div>
  </div>
</template>
